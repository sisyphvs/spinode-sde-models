...
###############################################################################
# Additional reconstruction functions (LVE, Black–Scholes, Heston)
#
# The functions below were added by J. Cruz Araneda and B. Lagos Guerra 
# for the numerical experiments in [Cruz Araneda, Lagos Guerra, 2025]. 
# They extend the original SPINODE implementation by providing:
#
#   - reconstruct_LVE:   reconstruction of drift/diffusion for the
#                        Lotka–Volterra system.
#   - reconstruct_BS:    reconstruction for the Black–Scholes model.
#   - reconstruct_Heston: reconstruction for the Heston stochastic
#                        volatility model.
#
# These functions are not part of the original implementation of this
# repository.
############################################################################### 

###############################################################################
def reconstruct_LVE(hp,
                    model, 
                    device,
                    sigma_mu, 
                    sigma_std, 
                    ...
                    path):
    """
    Function that reconstructs hidden physics for the LVE (Lotka-Volterra) system.
    """
    # State space grid
    numx_plot = 100
    numy_plot = 100
    x = np.linspace(0.1, 1.5, numx_plot)
    y = np.linspace(0.1, 1.5, numy_plot)

    g_pred = np.zeros((numx_plot, numy_plot, nx))
    g_true = np.zeros((numx_plot, numy_plot, nx))

    for i in range(numx_plot):
        for j in range(numy_plot):
            states = np.asarray([x[i], y[j], 0, 0])
            if hp == 'g1':
                _, g_true_ind, _ = dynamics.stoch_dyn_LVE(states)
            elif hp == 'g2':
                _, _, g_true_ind = dynamics.stoch_dyn_LVE(states)
            
            g_true[i, j, :] = g_true_ind.flatten()
            
            g_pred_ind = NN_eval(model, device, states[:nx+nu].reshape(1, nx+nu),
                                 sigma_mu, sigma_std, nx, dt)[0]
            g_pred[i, j, :] = g_pred_ind
    ...
    for k in range(nx):
        ...
        plt.contour(x, y, g_true[:, :, k], levels=50, colors='black', linestyles=':')
        plt.title(f"{hp.upper()} component {k+1}")
        plt.xlabel("x population")
        plt.ylabel("y population")
        plt.tight_layout()
        plt.savefig(f"{path}/{hp}_LVE_component{k+1}.png")
        plt.show()
        plt.close()
...
def reconstruct_BS(hp,
                   ...
                   path):
    """
    Function that reconstructs hidden physics for the Black-Scholes (GBM) model.
    
    hp --> 'g1' or 'g2'  (drift or diffusion term)
    model --> trained neural network
    device --> 'cuda' or 'cpu'
    sigma_mu/std --> standardization parameters
    nx --> state dimension (1 for BS)
    nu --> exogenous input dimension (0 for BS)
    dt --> time step (float)
    ...
    S_min, S_max = 10, 200
    numS = 200
    S_vals = np.linspace(S_min, S_max, numS)
    ...
    # Initialize arrays
    g_pred = np.zeros(numS)
    g_true = np.zeros(numS)

    # Evaluate for each S
    for i, S in enumerate(S_vals):
        states = np.asarray([S, 0])  # [Sk, Skw]
        
        # Get true value from analytical BS model
        if hp == 'g1':
            _, g_true_i, _ = dynamics.stoch_dyn_BS(states)
        elif hp == 'g2':
            _, _, g_true_i = dynamics.stoch_dyn_BS(states)
        g_true[i] = g_true_i[0]

        # NN prediction (standardized)
        g_pred_i = NN_eval(model, device, states[:nx+nu].reshape(1, nx+nu),
                           sigma_mu, sigma_std, nx, dt)[0]
        g_pred[i] = g_pred_i

    # Plot comparison
    plt.figure(figsize=(7,5))
    plt.plot(S_vals, g_true, 'k--', linewidth=2, label='True')
    plt.plot(S_vals, g_pred, 'r-', linewidth=2, label='SPINODE')
    plt.xlabel("Asset price $S$", fontsize=14)
    if hp == 'g1':
        plt.ylabel(r"$g_1(S) = \mu S$", fontsize=14)
    elif hp == 'g2':
        plt.ylabel(r"$g_2(S) = \frac{1}{2}(\sigma S)^2$", fontsize=14)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.grid(True)
    plt.savefig(f"{path}/{hp}_BS.png", dpi=150)
    plt.show()
    plt.close()

    # Compute RMSE
    rmse = (np.mean((g_pred - g_true)**2))**0.5
    np.save(f"{path}/rmse_{hp}_BS.npy", rmse)
    print(f"RMSE({hp}) = {rmse:.6e}")

    return rmse
###############################################################################

###############################################################################
def reconstruct_Heston(hp, model, device, sigma_mu, sigma_std, nx, nu, dt, path):
    """
    ...
    v_vals = np.linspace(0.01, 0.09, numV)

    g_pred = np.zeros((numS, numV))
    g_true = np.zeros((numS, numV))

    for i, S in enumerate(S_vals):
        for j, v in enumerate(v_vals):
            states = np.asarray([S, v, 0, 0])
            if hp == 'g1':
                _, g_true_i, _ = dynamics.stoch_dyn_Heston(states)
                g_true[i, j] = g_true_i[0]  # drift of price
            elif hp == 'g2':
                _, _, g_true_i = dynamics.stoch_dyn_Heston(states)
                g_true[i, j] = g_true_i[0]  # diffusion of price

            g_pred_i = NN_eval(model, device, states[:nx+nu].reshape(1, nx+nu),
                               sigma_mu, sigma_std, nx, dt)[0]
            g_pred[i, j] = g_pred_i[0] if isinstance(g_pred_i, (list, np.ndarray)) else g_pred_i

    fig, ax = plt.subplots(figsize=(7, 5))

    # Guardamos el objeto de contourf para usarlo en el colorbar
    cont = ax.contourf(S_vals, v_vals, g_pred.T, levels=40, cmap='viridis')

    # Contornos teóricos
    ax.contour(S_vals, v_vals, g_true.T, colors='k', linestyles='dashed')

    # Etiquetas y formato
    ax.set_xlabel("Asset price S")
    ax.set_ylabel("Variance v")
    ax.set_title(f"{hp.upper()} for Heston model")

    # Barra de color vinculada al mapa
    cbar = fig.colorbar(cont, ax=ax, shrink=0.85, pad=0.05)
    cbar.set_label(f"{hp}", fontsize=12)
    cbar.ax.tick_params(labelsize=10)

    # Ajuste final y guardado
    plt.tight_layout()
    plt.savefig(f"{path}/{hp}_Heston.png", dpi=150)
    plt.show()
    plt.close()

    rmse = np.sqrt(np.mean((g_pred - g_true)**2))
    np.save(f"{path}/rmse_{hp}_Heston.npy", rmse)
    print(f"RMSE({hp}) = {rmse:.4f}")
    return rmse
###############################################################################
